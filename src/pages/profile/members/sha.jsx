...
import shaImage from '../../../assets/members/sha.png';
import shaImage1 from '../../../assets/members/sha1.png';
...
const ShaProfile = () => {
  ...
  const handleSubmit = (e) => {
    ...
    window.location.href = `mailto:ic.shaniahjaer@gmail.com?subject=${encodeURIComponent(subject)}&body=${encodeURIComponent(body)}`;
  };
  ...
  return (
    <div className="min-h-screen w-full text-white font-sans flex flex-col bg-cover bg-center overflow-x-hidden">
      ...
      <main className="flex-grow w-full">
        <section className="w-full pt-16 pb-14 px-4 sm:px-6 md:px-10 lg:px-16 xl:px-28">
          <div className="flex flex-col-reverse md:flex-row items-center justify-between gap-12">
            ...
            <div className="opacity-0 animate-float-up delay-300 text-center md:text-left w-full">
              ...
              <div className="mt-6 sm:mt-6 md:mt-8 px-2 flex justify-center md:justify-start gap-3 sm:gap-4">
                {[{ type: "facebook", link: "https://www.facebook.com/share/1BZuubRGHd/", icon: facebookIcon },
                  ...
                  .map((social, i) => (
                    <a key={i} href={social.link} target="_blank" rel="noopener noreferrer" className="transition-transform duration-300 hover:scale-110">
                      <img src={social.icon} alt={social.type} className="w-7 h-7 sm:w-8 sm:h-8 md:w-9 md:h-9 lg:w-10 lg:h-10" />
                    </a>
                  ))}
              </div>
            </div>
            ...
            <div className="flex justify-center w-full opacity-0 animate-slide-in-left delay-400">
              <img src={shaImage} alt="Shaniah" className="h-auto max-h-[28rem] sm:max-h-[30rem] md:max-h-[35rem] w-auto object-contain rounded-lg" />
            </div>
          </div>
        ...
        {/* About Section */}
        <section ref={aboutRef} className={`w-full min-h-screen px-8 sm:px-10 md:px-14 xl:px-28 flex items-center justify-center ${showAbout ? 'animate-slide-in-up opacity-100' : 'opacity-0'}`}>
          <div className="flex flex-col md:flex-row items-center justify-between gap-14 w-full max-w-7xl pt-12 pb-14">
            {/* Profile Image */}
            <img ref={imageRef} src={shaImage1} alt="Avatar" className={`h-76 md:h-88 rounded-lg transition-all duration-1000 ${showImage ? 'animate-zoom-in-fade opacity-100' : 'opacity-0'}`} />
            ...
            <div className="max-w-3xl text-center md:text-left">
              ...
              <p className="text-gray-400 text-xl md:text-2xl leading-relaxed">
                I am a junior web developer who adores turning ideas into clean, responsive, and user-friendly websites. I am working primarily with HTML, CSS, and Bootstrap to build attractive designs, JavaScript to add interactivity.
              </p>
            </div>
          </div>
        </section>
        ...
        <section className="px-8 sm:px-10 md:px-14 xl:px-28 py-16 mt-28">
          ...
          <div className="flex gap-10 justify-center flex-wrap">
            {["HTML", "CSS", "Bootstrap"].map((title, index) => {
              const descriptions = {
                HTML: "HTML is the markup language used to structure and present content on web pages.",
                CSS: "CSS is used to style and format web pages by controlling the layout, colors, fonts, and overall design.",
                Bootstrap: "Bootstrap is a popular framework that simplifies web design by offering pre-made, responsive CSS and JavaScript components.",
              };
              ...
              return (
                <div key={index} className="w-96 h-112 perspective cursor-pointer" onClick={() => toggleFlip(index)}>
                  <div className={`relative w-full h-full transform-style-preserve-3d transition-transform duration-700 ${flippedIndex === index ? 'rotate-y-180' : ''}`}>
                    {/* Front */}
                    <div className="card-face backface-hidden flex flex-col items-center justify-center text-white" style={{ backgroundImage: `url(${cardImage})`, backgroundSize: 'cover', backgroundPosition: 'center' }}>
                      <img src={imgs[title]} alt={title} className="h-24 mb-4" />
                      <h3 className="text-lg font-bold">{title}</h3>
                    </div>

                    {/* Back */}
                    <div className="card-face rotate-y-180 backface-hidden flex items-center justify-center text-white p-8" style={{ backgroundImage: `url(${cardImage})`, backgroundSize: 'cover', backgroundPosition: 'center' }}>
